...
- 配布・同期のロジックは [`hooks/`](./hooks) にある3本のフックスクリプトが担う:
  ...
  - **`hooks/archive-turn.py`**(`Stop`フック、Claudeが応答を終えるたび=毎ターン)
    ...
    (five時間利用制限の監視用ログ)。**閾値超えでの自動通知は現時点で無効化している**
    (`NOTIFY_ENABLED = False`) — 実装時にローカル集計とアカウント側の実数値との間に約2.1倍の
    未解明の乖離が見つかったため、根拠の薄い閾値で通知を出すより、まずログだけ溜める方針にした。
    詳細は`NOTES-2026-08-21-handoff.md`を参照。
